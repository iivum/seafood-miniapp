# Spec: order-customer-state-machine

## Purpose

[TBD — see change `v2-visual-redesign` for context. Defines the customer-side order state machine, the action endpoints, the REFUNDING/REFUNDED terminal states, the Refund sub-aggregate, and the tracking value object.]
