# Spec: property-testing

## Purpose

核心 domain 不变量的 property-based 测试:库接入(jqwik,与 junit-platform 6 并存)、首批覆盖的不变量(`Product.decrementStock` 数值边界、`OrderStatus` 状态机、`Sku`/`Product` 构造校验)、随机生成器约束、以及与 example-based 测试的边界。example 测试只验有限点位,property 用随机样本逼反例。源自 change `sprint-5-c4-property-testing`(Sprint 5 C4)。
