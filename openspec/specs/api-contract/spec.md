...
后端 HTTP API 的 OpenAPI 3 spec 作为后端↔双 TS 前端(微信小程序、admin-ui)契约的 SoT:生成方式(springdoc,test-scope 不污染 native)、committed SoT + 漂移门、响应一致校验、以及前端消费启用。变异/覆盖率看不见的「实现偏离声明」由本契约接住。源自 change `sprint-5-c2-openapi-contract`(Sprint 5 C2;形态选 OpenAPI 而非 Spring Cloud Contract —— SCC 对「1 JVM producer + 2 TS consumer」拓扑是错工具)。
