# Spec: runtime-security

## Purpose

[TBD — see change `sprint2-native-security` for context. Defines the runtime security guarantees enforced at the HTTP layer: baseline security headers on every response, and a fixed-window rate limit on the admin BFF surface.]
