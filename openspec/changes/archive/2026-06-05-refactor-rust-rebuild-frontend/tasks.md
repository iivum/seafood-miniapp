...
- [x] 8.1 Restructure `frontend/src/` from type-based (`pages/`, `utils/`, `types/`) to feature-based (`features/{product,cart,order,user,admin}/`) + `shared/`
- [x] 8.2 Move all `.wxml/.wxss/.ts` pages into `pages/` and wire them to consume `features/*/components`
- [x] 8.3 Centralize `shared/api/request.ts` with auto token-refresh on 401
- [x] 8.4 Implement `features/auth/store.ts` for WeChat login flow (`wx.login` → `POST /api/auth/wechat-login`)
- [x] 8.5 Wire `features/cart` and `features/order` pages to new cart/order endpoints
- [x] 8.6 Add the shared `tokens.json` consumer so WXSS reads `var(--color-primary)` etc. (CI-synced with admin-ui)

## 9. Admin UI (new stack)

- [x] 9.1 Delete old `backend/admin-ui/` Vue 3 + Element Plus sources
- [x] 9.2 Scaffold new top-level `admin-ui/` with Vite + React 18 + TypeScript strict
- [x] 9.3 Initialize shadcn/ui (`npx shadcn@latest init`) and add primitives (Button, Card, Dialog, Form, Input, Select, Table)
- [x] 9.4 Configure Tailwind to consume shared `tokens.json` for color/spacing/typography
- [x] 9.5 Build `Login` view + httpOnly-cookie auth flow against `/api/admin/auth/login`
- [x] 9.6 Build `Dashboard` view consuming `GET /api/admin/dashboard`
- [x] 9.7 Build `ProductList` + `ProductForm` views against `/api/products/**` and `/api/admin/products/stats`
- [x] 9.8 Build `OrderList` + `OrderDetail` views against `GET /api/orders` and `GET /api/admin/orders/{id}/detail`
- [x] 9.9 Configure route protection (redirect anonymous to `/admin/login`)
