...
- [x] 4.1.1 跑 `./gradlew clean check nativeTest nativeCompile` 全链路绿
- [x] 4.1.2 本地 `docker-compose down -v && docker-compose up -d --build`
- [x] 4.1.3 等待 backend healthy,跑 `backend/scripts/native-smoke.sh` 全套(health / prometheus / RSS / latency)
- [x] 4.1.4 触发完整用户旅程后,导出 `curl http://localhost:9090/actuator/prometheus > /tmp/metrics.txt`,人工 review 含全部 5 个业务 counter + http_server_requests + jvm.* + mongodb.driver.commands
- [x] 4.1.5 验证日志:`docker logs backend 2>&1 | head -20 | jq .`(每行可解析为 JSON,含 `@timestamp` / `level` / `message` / `requestId`)
- [x] 4.1.6 验证 RSS:`docker exec backend cat /proc/1/status | grep VmRSS`,< 204800 KB

### 4.2 文档同步

- [x] 4.2.1 在 `CLAUDE.md` 加"可观测性"段落:
...
- [x] 4.2.2 在 `README.md` 加"Operations"段落:scrape 端点示例 `prometheus.yml` 片段 + 日志 schema 字段表
- [x] 4.2.3 在 `backend/scripts/native-smoke.sh` 顶部加注释说明新增的 prometheus + RSS 断言
- [x] 4.2.4 在 `docs/runbooks/`(若尚未存在,从 Task #5 复用模板)写一个 mini-runbook:`/actuator/prometheus 不可达` 的排查步骤

### 4.3 PR 收口

- [x] 4.3.1 commit `chore(observability): end-to-end verification & docs`
- [x] 4.3.2 push `feat/observability-stack` 到 origin
- [x] 4.3.3 用 `gh pr create --base main --title "feat(observability): metrics endpoint + structured logging + business counters" --body-file <<<...` 创建 PR,body 引用 `openspec/changes/setup-observability-stack/{proposal,design}.md`
- [x] 4.3.4 PR 描述末尾加 `🤖 Generated with [Claude Code](https://claude.com/claude-code)`(项目 PR 约定)
- [x] 4.3.5 等 CI 全绿(jvm-check / native / security 三个 workflow)

## 5. Release & Cleanup

- [x] 5.1 PR review 通过后,squash merge 到 main
- [x] 5.2 跑 `openspec apply setup-observability-stack`(或对应 `/opsx:apply` 流程)归档 change
- [x] 5.3 跑 `openspec sync`(若需要)把 `metrics-export` 与 `structured-logging` 两个 spec 提升到 `openspec/specs/`
- [x] 5.4 通过 `git worktree remove .worktrees/setup-observability-stack` 清理 worktree(或用 ExitWorktree action=remove)
- [x] 5.5 TaskUpdate #1 → `completed`;TaskList 确认 #7、#8 不再 blocked
- [x] 5.6 给主 session 一份"实施完成 + 关键回顾"短报告,触发 Task #7 决策环节
