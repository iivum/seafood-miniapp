...
- [x] 2.5 Implement token-bucket `AdminRateLimiter` using Caffeine `Cache<String, AtomicLong>` keyed by `clientIp + ":" + account` with refill every 60 s — `backend/src/main/java/com/seafood/shared/security/AdminRateLimiter.java`; covered by `AdminRateLimiterTest` (uses `Ticker` to fast-forward time).
