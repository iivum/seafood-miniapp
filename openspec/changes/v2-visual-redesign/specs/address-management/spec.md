...
The system SHALL render an address edit screen at the path `pages-sub/user/address/address-edit` accepting fields: name, phone, province (picker), city (picker, filtered by province), district (picker, filtered by city), detail (textarea), and a "设为默认" toggle. On save, the screen MUST call `POST /api/users/me/addresses` for new addresses or `PUT /api/users/me/addresses/{id}` for existing ones. Field validation: name 1-30 chars, phone 11-digit regex, province/city/district required, detail 1-100 chars.

#### Scenario: Edit an existing address
- **WHEN** the user navigates to the edit screen with an address id in the query
- **THEN** the screen loads the existing address via `GET /api/users/me/addresses/{id}` (or from list payload)
- **AND** pre-fills all fields
- **AND** on save calls `PUT /api/users/me/addresses/{id}` with the modified payload

#### Scenario: Validation error on empty phone
- **WHEN** the user taps "保存" with an empty phone field
- **THEN** the system highlights the phone field in error state (red border)
- **AND** shows the message "请输入手机号"
- **AND** does not submit the form

#### Scenario: Save success
- **WHEN** the user submits a valid edit form
- **THEN** the system calls the appropriate POST or PUT
- **AND** on success pops the page back to the address list
