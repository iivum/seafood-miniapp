/**
 * category.wxml ↔ category.js bindtap 契约(防死绑定回归)。
 *
 * 背景(mp-cross-screen-cleanup change,design.md D2):这次改动把 category.js
 * 的 `addToCart` 重命名为 `onAddToCart`,必须同步改 category.wxml 里
 * `catchtap="addToCart"` → `catchtap="onAddToCart"`,否则分类页商品卡片的
...
const WXML_PATH = path.resolve(__dirname, '../category.wxml');
...
jest.mock('../../../src/modules/productList/productList.js', () => ({
  ProductListModule: jest.fn().mockImplementation(() => ({
    ...
    state: { products: [], isLoading: false, isError: false },
    isEmpty: false,
    isLoading: false,
    hasNext: false,
    getErrorMessage: () => '',
...
describe('category.wxml ↔ category.js bindtap 契约(防死绑定回归)', () => {
  let wxml;
  ...
  it('wxml 里每个 bindtap/catchtap 目标都必须是 category.js 里真实存在的方法', () => {
